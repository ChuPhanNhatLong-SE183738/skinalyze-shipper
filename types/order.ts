 ...
 */
export const convertAPIOrderToAppOrder = (apiLog: ShippingLogResponse): Order => {
  ...
  const items = hasOrderItems && (apiLog.order as any).orderItems?.map((item: any) => ({
    id: item.orderItemId,
  ...
  return {
    ...
    transactionStatus: 'transaction' in apiLog.order ? (apiLog.order as any).transaction?.status : undefined,
  };
};
